...
jQuery(document).ready(function(){
    ...
    jQuery(".allergen_check").change(function(){
        checkTerms(this);
        // if(jQuery(this).prop('checked')){
        //     jQuery("#ch0").prop("checked", false);
        //     jQuery(this).parent().find("input[name='"+jQuery(this).attr('real_name')+"']")[0].value=1;
            
        // } else {
        //     jQuery(this).parent().find("input[name='"+jQuery(this).attr('real_name')+"']")[0].value=0;
        // }
        // return true;
    })
});
function checkTerms(checkObj) {

	if(jQuery(checkObj).prop('checked')){
        jQuery("#ch0").prop("checked", false);
        jQuery(checkObj).parent().find("input[name='"+jQuery(checkObj).attr('real_name')+"']")[0].value=1;
    } else {
        jQuery(checkObj).parent().find("input[name='"+jQuery(checkObj).attr('real_name')+"']")[0].value=0;
    }
}
</script>
</head>
                            ...
                            echo '<tr id="'.$category->allergen_id.'">';
                                ...
                                if(isset($allergens_array[$category->allergen_id])){
                                    foreach ($allergens_array[$category->allergen_id] as $allergen_sub_array) {
                                        ...
                                        echo '<input class="allergen_check" id="sub_'.$allergen_sub_array[0].'" type="checkbox" real_name="sub_'.$allergen_sub_array[0].'_'.$allergen_sub_array[1].'" checked  />
                                                <label for="sub_'.$allergen_sub_array[0].'">'.$allergen_sub_array[1].'</label>
                                                <input type=hidden name="sub_'.$allergen_sub_array[0].'_'.$allergen_sub_array[1].'"  value="1" >
                                                ';
                                        
                                    }
                                } else {
                                    // $checked = array_search('parent_'.$category->allergen_id, $saved_data_array)>-1?"checked":"" ;
                                    echo '<input class="allergen_check" id="parent_'.$category->allergen_id.'" type="checkbox" real_name="parent_'.$category->allergen_id.'_'.$category->allergen_name.'" checked />
                                            <label for="parent_'.$category->allergen_id.'">'.$category->allergen_name.'</label>
                                            <input type=hidden name="parent_'.$category->allergen_id.'_'.$category->allergen_name.'" value="1">
                                            ';
                                }
