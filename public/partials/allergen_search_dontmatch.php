        ...
        if($_POST['flag']==1){
            ...
            if($_POST['match'] == 1) {
                ...
                $sql = 'SELECT item_allergens_item as parentstr, allergen_name as childstr 
                        ...
                        where item_allergens_item in ("'.$in_array.'") and allergen_value=1';
                
            } else {
                $p2 = "DON'T MATCH what I entered";
                $sql = 'SELECT item_allergens_item as parentstr, allergen_name as childstr 
                        FROM `haccp_item_allergens` 
                        where item_allergens_item in ("'.$in_array.'") and allergen_value=0';
            ...
            if($_POST['match'] == 1) {
                ...
                $sql = 'SELECT allergen_name as parentstr, item_allergens_item as childstr 
                    FROM `haccp_item_allergens` 
                    where allergen_name in ("'.$in_array.'") and allergen_value=1';
            
            } else {
                $p2 = "DON'T MATCH what I entered";
                $sql = 'SELECT allergen_name as parentstr, item_allergens_item as childstr 
                    FROM `haccp_item_allergens` 
                    where allergen_name in ("'.$in_array.'") and allergen_value=0';
...
<form method="POST" action="" id="myform">
    ...
    <div class="row">
        ...
        <div class="col-sm-6">
            <div class="row">
                <label for="match">The results should show all items which:</label>
            </div>
            <div class="row">
                <select class="form-control" id="match" name="match">
                    <option value="1">MATCH what I entered</option>
                    <option value="2">DON'T MATCH what I entered</option>
                </select>
            </div>
        </div>
    </div>
